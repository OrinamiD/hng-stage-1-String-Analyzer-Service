...
// get specific string
export const stringValue = async (value) => {
...
export const parseNaturalLanguageQuery = (query) => {
    ...
    if (lowerQuery.includes("single word") || lowerQuery.includes("one word")) {
        ...
        const explicitWordMatch = lowerQuery.match(/(\d+)\s*(?:-?\s*)?(?:words?|word)/);
    ...
    const longerMatchAlt = lowerQuery.match(/greater than\s+(\d+)\s*(?:characters|chars)?/);
    ...
    if (shorterMatch && shorterMatch[1]) {
        ...
        if (!Number.isNaN(n))
            filters.max_length = n - 1;
    }
    // ✅ contains the letter x OR contain x
    const containsMatch = lowerQuery.match(/contain(?:s|ing)?(?: the letter)?\s+([a-z])/);
...
// Delete String
export const deleteByValue = async (value) => {
